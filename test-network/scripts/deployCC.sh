...
if [ "$CC_SRC_PATH" = "NA" ]; then
  ...
  if [ "$CC_NAME" = "basicuser" ]; then
    println $'\e[0;32m'user-basic$'\e[0m' chaincode
    CC_SRC_PATH="../user-basic"
  else
    fatalln "The chaincode name ${CC_NAME} is not supported by this script. Supported chaincode names are: basicuser"
  fi
