...
if [[ $# -ne 1 ]]; then
    echo "Need 1 parameters: ./revokeuser.sh Authority"
    echo "- Authority   : Either org1 or org2"
    echo "  This script will revoke the ability to update the ledger from user in either 'org1/org2' "
    exit 2
fi

if [[ "$1" = "org1" ]]
then
    echo "Revoke user from 'org1' to update the ledger"
    export FABRIC_CA_CLIENT_HOME=${PWD}/organizations/peerOrganizations/org1.example.com/

    fabric-ca-client identity modify  org1admin --attrs 'role=nomal:ecert' --tls.certfiles ${PWD}/organizations/fabric-ca/org1/tls-cert.pem
...
then
    echo "Revoke user from 'org2' to update the ledger"
    export FABRIC_CA_CLIENT_HOME=${PWD}/organizations/peerOrganizations/org2.example.com/

    fabric-ca-client identity modify  org2admin --attrs 'role=normal:ecert' --tls.certfiles ${PWD}/organizations/fabric-ca/org2/tls-cert.pem
